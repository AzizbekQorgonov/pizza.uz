...
function KomboPage() {
  ...
  async function getProducts() {
    try {
      ...
      // Kombo kategoriyasini topish
      const komboCategory = categoriesRes.data.find(
        (cat) =>
          cat.title.toLowerCase().trim().includes("комбо") ||
          cat.title.toLowerCase().trim().includes("combo")
      );

      if (komboCategory) {
        // Mahsulotlarni olish va Kombo bilan filtrlash
        const productsRes = await axios.get(
          "https://693d1ae6f55f1be79301e90f.mockapi.io/products"
        );
        const komboProducts = productsRes.data.filter(
          (p) => String(p.categoryId) === String(komboCategory.id)
        );
        setProducts(komboProducts);
      } else {
        setProducts([]);
      }
    } catch (err) {
